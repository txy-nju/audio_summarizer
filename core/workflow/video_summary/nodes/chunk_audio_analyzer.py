...
def _load_transcript_data(transcript: str) -> Dict:
...
def _build_transcript_items(transcript_data: Dict) -> List[Dict]:
    items: List[Dict] = []
...
def _extract_chunk_text(transcript_items: List[Dict], indexes: List[int]) -> str:
    lines: List[str] = []
    for idx in indexes:
...
def _search_with_cache(query: str) -> str:
    if ENABLE_CHUNK_CACHE and query in _AUDIO_SEARCH_CACHE:
        return _AUDIO_SEARCH_CACHE[query]
...
def chunk_audio_analyzer_node(state: VideoSummaryState) -> dict:
    ...
    for chunk in chunk_plan:
        ...
        merged = result_map.get(chunk_id, {"chunk_id": chunk_id})
        merged.update(
            {
                ...
                "token_usage": {
                    **(merged.get("token_usage", {}) if isinstance(merged.get("token_usage", {}), dict) else {}),
                    "audio": 0,
                },
                "latency_ms": {
                    **(merged.get("latency_ms", {}) if isinstance(merged.get("latency_ms", {}), dict) else {}),
                    "audio": latency_ms,
                },
            }
        )
